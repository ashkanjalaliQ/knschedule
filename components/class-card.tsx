...
const ClassCard: React.FC<ClassCardProps> = ({ classItem, index, isToday, day }) => {
  ...
  return (
    <div
      className={`bg-gray-50 rounded-lg p-4 dark:bg-gray-800 hover:bg-gray-100 ${isCurrentClass(classItem) && isToday ? 'ring-2 ring-indigo-500' : ''}`}
    >
      ...
      <div className="space-y-2 text-sm text-gray-600 dark:text-white">
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            className="px-3 py-1 text-xs bg-indigo-50 hover:bg-indigo-100 dark:bg-indigo-900/50 dark:text-indigo-200 hover:shadow-md text-indigo-600 rounded-full focus:ring-1 focus:ring-offset-1 focus:ring-indigo-500 flex items-center group"
