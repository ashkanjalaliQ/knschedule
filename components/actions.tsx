...
const Actions: React.FC<ActionsProps> = ({ gender, setGender }) => {
  ...
  const handleGenderChange = (newGender: 'boys' | 'girls') => {
    saveGenderToLocalStorage(newGender);
    setGender(newGender);
    setIsOpen(false);
  };
  ...
  const getGenderIcon = (type: 'boys' | 'girls', isButton = false) => {
    if (type === 'girls') {
      return <FaFemale className={`${isButton ? 'text-xl' : 'text-base'} ${gender === 'girls' ? 'text-white' : 'text-pink-400'}`} />;
    }
    return <FaMale className={`${isButton ? 'text-xl' : 'text-base'} ${gender === 'boys' ? 'text-white' : 'text-blue-400'}`} />;
  };
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      <div className="flex items-center justify-center">
        <div className="flex items-center space-x-2 rtl:space-x-reverse bg-white/10 backdrop-blur-sm 
      ...
      <div className={`absolute top-full left-1/2 transform -translate-x-1/2 mt-2 
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-3 min-w-[160px]
          ...
          <div className="flex flex-col space-y-2">
            ...
            >
              <span className={`font-medium ${gender === 'girls' ? 'text-white' : 'text-gray-700 dark:text-gray-200'}`}>
                دختران
              </span>
            ...
            >
              <span className={`font-medium ${gender === 'boys' ? 'text-white' : 'text-gray-700 dark:text-gray-200'}`}>
                پسران
              </span>
